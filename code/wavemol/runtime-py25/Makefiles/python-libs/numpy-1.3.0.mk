...
$(NUMPY_TARGET): $(INIT_TARGET) $(DOWNLOAD_DIR)/$(NUMPY_PACKAGE)
	...
	cd $(UNPACK_DIR)/numpy-$(NUMPY_VERSION); \
	 export PATH=$(RUNTIME_DIR)/bin:$$PATH LDFLAGS="$(LIBS):$(LDFLAGS)" CPPFLAGS="$(INCLUDES):$(CPPFLAGS)" PYTHONPATH=$(RUNTIME_DIR)/lib/python2.5/site-packages/; \
	 $(RUNTIME_DIR)/bin/python2.5 setup.py build --fcompiler=gnu95; \
	 $(RUNTIME_DIR)/bin/python2.5 setup.py install --prefix=$(RUNTIME_DIR)
	touch $(NUMPY_TARGET)
endif
