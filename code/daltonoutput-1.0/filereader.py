...
class FileReader(file):
	""" 
	extended file class to access smoothly the lines 
	as when using readlines(), with no memory payload
	FIXME!!!: this class may behave wrongly if fed up
	with an empty file. Check this opportunity!
	"""	
	...
	def __getitem__(self,key):
		...
		file.seek(self,self.__lineToFseek[key],0)
		counter=0
		line = file.readline(self)
	...
	def findRegexp(self, theregexp, count=1, backward=0):
		...
		returns a tuple. The fist element is the matched line. The other element
		is a tuple that contains the matched groups, or an empty tuple if no group tags
		were specificed
		"""
