...
class TestGraph(unittest.TestCase):
    ...
    def testGetLinks(self): # fold>>
        ...
        node3 = graph.createEntity()
        node4 = graph.createEntity()
        node5 = graph.createEntity()
        node6 = graph.createEntity()
        link = graph.createEntity(entities=(node1, node2))
        link = graph.createEntity(entities=(node4, node5))
        link = graph.createEntity(entities=(node5, node4))

        self.assertEqual(len(graph.getLinks(node1)), 1)
        self.assertEqual(len(graph.getLinks(node4)), 2)
