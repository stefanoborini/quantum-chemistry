...
class CenterOfMassToken(object):
    ...
    def match(cls, reader, previous_tokens):  # fold>>
        ...
        if m is not None:
            return cls(  [float(m.group(1)), float(m.group(2)), float(m.group(3))] * units.bohr)
...
class TotalMassToken(object):
    ...
    def match(cls, reader, previous_tokens):  # fold>>
        ...
        if m is not None:
            return cls( float(m.group(1)) * units.dalton )

        reader.toPos(start_pos)
        return None
    # <<fold

class IsotopicMassesToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        for line in reader:
            ...
            if m is not None:
                values.append( (m.group(1), m.group(2), float(m.group(3)) * units.dalton ) )
                contents = True
                continue
    
            m = re.search("(\w+)\s+(\d*\.\d*)", line)
            if m is not None:
                values.append( (  m.group(1), "", float(m.group(2)) * units.dalton ) )
                contents = True
                continue
            break
...
class MomentsOfInertiaToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        for line in reader:
            ...
            if m is not None:
                moments_of_inertia.append( float(m.group(1)) * units.angstrom * units.angstrom * units.dalton)
                principal_axes.append( [float(m.group(2)), float(m.group(3)), float(m.group(4))] * units.angstrom)
                contents = True
                continue
            break
...
class CartesianCoordinatesToken(object):
    def __init__(self, atom_list): # fold>>
        self._atom_list = atom_list
    # <<fold 
    def atomList(self): # fold>>
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        while True:
            ...
            atom_list.append( (label, sym_label, [x_coord, y_coord, z_coord] * units.bohr ))
...
class EndOfOptimizationHeaderToken(object):
    def __init__(self): # fold>>
        pass
    # <<fold
    @classmethod
...
class FinalGeometryEnergyToken(object):
...
class GeometryConvergenceNumIterationsToken(object):
...
class OptimizationNextGeometryToken(object):
    def __init__(self, atom_list): # fold>>
        self._atom_list = atom_list
    # <<fold 
    def atomList(self): # fold>>
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        for line in reader:
            ...
            if m1 is not None:
                ...
                sym_label = m1.group(2)
                position = [float(m1.group(3)), float(m1.group(4)), float(m1.group(5))] * units.bohr
            ...
            if m2 is not None:
                ...
                sym_label = ""
                position = [ float(m2.group(2)), float(m2.group(3)), float(m2.group(4))] * units.bohr
...
class OptimizationInfoToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        if m3 is not None:
            energy = float(m3.group(1)) * units.hartree
        else:
            reader.toPos(start_pos)
            return None
        ...
        if m4 is not None:
            energy_change = float(m4.group(1))*units.hartree
        else:
            energy_change = None
            reader.readbackline()
        ...
        if m5 is not None:
            gradient_norm = float(m5.group(1)) * units.unknown
        else:
            reader.toPos(start_pos)
            return None
        ...
        if m6 is not None:
            step_norm = float(m6.group(1)) * units.unknown
        else:
            reader.toPos(start_pos)
            return None
        ...
        if m7 is not None:
            trust_radius = float(m7.group(1)) * units.unknown
        else:
            reader.toPos(start_pos)
            return None
...
class NormalModesEigenvaluesToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        for index, value in zip(all_columns, all_values):
            output[index-1] = value * units.unknown
...
class AtomsAndBasisSetsToken(object):
...
class FinalGeometryToken(object):
    def __init__(self, atom_list): # fold>>
        self._atom_list = atom_list
    # <<fold 
    def atomList(self): # fold>>
...
class DipoleMomentToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        try:
            dipole = float(m1.group(2)) * units.debye
        except ValueError:
            reader.toPos(start_pos)
            return None
...
class DipoleMomentComponentsToken(object):
    def __init__(self, dipole): # fold>>
        self._dipole= dipole
    # <<fold 
    def dipole(self): # fold>>
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        return cls([dipole_x, dipole_y, dipole_z] * units.debye)

    # <<fold 
 
class HOMOLUMOSeparationToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        try:
            lumo_energy = float(m1.group(1)) * units.hartree
            lumo_symmetry = int(m1.group(2))
            homo_energy = float(m2.group(1)) * units.hartree
            homo_symmetry = int(m2.group(2))
            gap = float(m3.group(1)) * units.hartree
        except ValueError:
            reader.toPos(start_pos)
            return None
...
class BondLengthsToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        for line in reader:
            ...
            if m5 is not None:
                atom_list.append( ( (m5.group(1), m5.group(2)) , (m5.group(3), m5.group(4)) , float(m5.group(5)) * units.angstrom) )
                valid_data = True
                continue
            ...
            if m6 is not None:
                atom_list.append( ( (m6.group(1), m6.group(2)) , (m6.group(3), "") ,  float(m6.group(4)) * Units.angstrom ) )
                valid_data = True
                continue
            ...
            if m7 is not None:
                atom_list.append( ( (m7.group(1), "") , (m7.group(2), m7.group(3)) ,  float(m7.group(4)) * Units.angstrom) ) 
                valid_data = True
                continue
            ...
            if m8 is not None:
                atom_list.append( ( (m8.group(1), "") , (m8.group(2), "") ,  float(m8.group(3)) * Units.angstrom ) )
                valid_data = True
                continue
...
class ResponseHeaderToken(object):
    def __init__(self): # fold>>
        pass
    # <<fold
    @classmethod
...
class FirstHyperpolarizabilityComponentToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        if m1 is not None:
            b_freq = float(m1.group(1)) * units.hartree
            c_freq = float(m1.group(2)) * units.hartree
            components = (m1.group(3), m1.group(4), m1.group(5))
            beta = float(m1.group(6)) * units.atomic_unit_of_1st_hyperpolarizability
            refers_to = None
        elif m2 is not None:
            b_freq = float(m2.group(1)) * units.hartree
            c_freq = float(m2.group(2)) * units.hartree
...
class SecondHyperpolarizabilityToken(object):
    ...
    def DFreq(self): # fold>>
        return self._d_freq
        # <<fold
    def gamma(self, A, B, C, D): # fold>>
        try:
            return self._gamma[ (A,B,C,D) ]
        except KeyError:
            return None
            # <<fold
    def allGamma(self): # fold>>
        return self._gamma
        # <<fold
    def averagedGamma(self): # fold>>
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        b_freq = float(m3.group(1)) * units.hartree
        ...
        c_freq =float(m4.group(1)) * units.hartree
        ...
        d_freq =  float(m5.group(1)) * units.hartree
        ...
        for line in reader:
            ...
            if m6 is not None:
                gamma[ (m6.group(1), m6.group(2), m6.group(3), m6.group(4)) ] = float(m6.group(5)) * units.atomic_unit_of_2nd_hyperpolarizability
                valid_data = True
                continue
            break
        ...
        if m7 is not None:
            averaged_gamma = float(m7.group(1)) * units.atomic_unit_of_2nd_hyperpolarizability
        else:
            averaged_gamma = None
...
class LinearResponseToken(object):
    ...
    def match(cls, reader, previous_tokens): # fold>>
        ...
        a_freq = float(m3.group(3)) * units.hartree
        ...
        b_freq = float(m4.group(3)) * units.hartree
        ...
        alpha = float(m5.group(1)) * units.atomic_unit_of_electric_polarizability

        return cls(a_freq, b_freq, [a_component, b_component], alpha)
    # <<fold

class SevereErrorToken(object):
