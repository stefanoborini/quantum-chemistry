...
class CartesianCoordinatesToken:
    ...
    def match(cls, reader): # fold>>
        ...
        while True:
            m1 = re.search("\s+\d+\s+(\S+)\s+(\S+)\s+x\s+(-?\d+\.\d+)", line)
            m2 = re.search("\s+\d+\s+(\S+)\s+x\s+(-?\d+\.\d+)", line)
...
class OptimizationNextGeometryToken:
    ...
    def match(cls, reader): # fold>>
        ...
        for line in reader:
            m1 = re.search("\s+(\S+)\s+(\S+)\s+(-?\d+\.\d+)\s+(-?\d+\.\d+)\s+(-?\d+\.\d+)", line)
            if m1 is not None:
            ...
            m2 = re.search("\s+(\S+)\s+(-?\d+\.\d+)\s+(-?\d+\.\d+)\s+(-?\d+\.\d+)", line)
            if m2 is not None:
...
class AtomsAndBasisSetsToken:
    ...
    def match(cls, reader): # fold>>
        ...
        line = reader.readline()
        atom_data_list = []
        regexp=re.compile("^\s*(\S+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\[.*\])\s+")
        # example line
...
class BondLengthsToken:
    ...
    def match(cls, reader): # fold>>
        ...
        for line in reader:
            ...
            m5 = re.search("bond distance:\s+(\S+)\s+(\d)\s+(\S+)\s+(\d)\s+(\d+\.\d+)", line)
            if m5 is not None:
            ...
            m6 = re.search("bond distance:\s+(\S+)\s+(\d)\s+(\S+)\s+(\d+\.\d+)", line)
            if m6 is not None:
            ...
            m7 = re.search("bond distance:\s+(\S+)\s+(\S+)\s+(\d)\s+(\d+\.\d+)", line)
            if m7 is not None:
            ...
            m8 = re.search("bond distance:\s+(\S+)\s+(\S+)\s+(\d+\.\d+)", line)
            if m8 is not None:
