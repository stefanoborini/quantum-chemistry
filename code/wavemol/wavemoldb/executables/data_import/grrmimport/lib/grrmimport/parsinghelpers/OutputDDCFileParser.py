    ...
    def _parseDDCTokenList(self):
        ...
        for t in tokens:
            ...
            if t.__class__ == tokentypes.StructureHeaderToken:
                ...
                current_molecule = grrm2.BarrierDissociated.new(self._graph,rdflib.URIRef("urn:uuid:"+str(uuid.uuid4())))
                self._molecules.append(current_molecule)
                grrm2.structureNumber(current_molecule).set(int(t.number()))
                
                self._struct_label_to_molecule_mapper[("dDC", t.number())] = current_molecule
