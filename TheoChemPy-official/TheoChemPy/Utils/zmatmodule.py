...
class ZMat2XYZ:
    """
    ZMat2XYZ 1.0.1 - By Stefano Borini
    ...
    * basis-string
    ...
    def parseFile(self,infile): # fold>>
        ...
        self.__zmatrix, self.__distance_units, self.__angle_units = _parseZMatrixSection(self.__infile, self.__parameters)
        self.__basis=_parseBasisSection(self.__infile, self.__zmatrix)
        self.__symmetries=_parseSymmetrySection(self.__infile)
    # <<fold 
    def writeDalton(self,filename): # fold>>
        """
        this routine prints the dalton .mol format for the given molecule
        """
        from string import join

        cartcoord = _cartesianCoords(self.__zmatrix, self.__basis)
        ...
        for k,v in groups.items():
            ...
            element = PeriodicTable.getElementBySymbol(atomsymbol)
    ...
    # <<fold 
    def writeDalton20(self, filename): # fold>>
        """
        this routine prints the dalton .mol format for the given molecule
        ...
        d.setSymmetryGenerators(self.__symmetries)
        atom_counter = {}
        for entry in cartcoord:
            ...
            if atom_counter.has_key(symbol):
                ...
                atom_counter[symbol] = 1
            d.addAtom(symbol+str(atom_counter[symbol]), PeriodicTable.getElementBySymbol(symbol), coords, self.__distance_units, basis)

        # ok... now produce the .mol input...
        
        f=file(filename,"w")
        f.write(d.generateOutput())
        f.close()
    # <<fold 
...
def _parseZMatrixSection(f, parameters): # fold>>
    ...
    if m.group(1) in ["Angstrom", "Ang"]:
        distance_unit=Units.Angstrom
    elif m.group(1) in ["AtomicUnits", "AU", "Bohr"]:
...
def _cartesianCoords(zmatrix, basis): # fold>>
    ...
    for currentAtom in zmatrix:
        ...
        if zmatrix.index(currentAtom) == 0:
            ...
            cart.append( (currentAtom[AtomListSymbol], Math.Vector(0.0,0.0,0.0),  _basis))
            ...
            if basis.has_key( int(currentAtom[AtomListId]) ):
                ...
                _basis = ''
            #print '-> _basis ',_basis,' ', int(currentAtom[AtomListId]) 
            cart.append( (currentAtom[AtomListSymbol], Math.Vector(0.0, 0.0, float(currentAtom[AtomListBondValue])), _basis))
            ...
            vec=Math.Vector(0.0, 0.0, float(currentAtom[AtomListBondValue]))
