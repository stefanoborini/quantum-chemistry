...
$(OPENBABEL_TARGET): $(INIT_TARGET) $(DOWNLOAD_DIR)/$(OPENBABEL_PACKAGE) 
	...
	cd $(UNPACK_DIR)/openbabel-$(OPENBABEL_VERSION)/scripts/python; export PATH=$(RUNTIME_DIR)/bin:$$PATH PYTHONPATH=$(RUNTIME_DIR)/lib/python2.4/site-packages/; $(RUNTIME_DIR)/bin/python2.4 setup.py install --prefix=$(RUNTIME_DIR)
...
$(OPENBABEL_TARGET): $(INIT_TARGET) $(DOWNLOAD_DIR)/$(OPENBABEL_PACKAGE) 
	-rm -rf $(UNPACK_DIR)/openbabel-$(OPENBABEL_VERSION)/
	tar -m -C $(UNPACK_DIR) -xzvf $(DOWNLOAD_DIR)/$(OPENBABEL_PACKAGE)
	cd $(UNPACK_DIR)/openbabel-$(OPENBABEL_VERSION); export CFLAGS="$$CFLAGS -m64" FC="g95 -m64" FCFLAGS="$$FCFLAGS -m64" LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:$(RUNTIME_DIR)/lib PATH=$(RUNTIME_DIR)/bin:$$PATH PYTHONPATH=$(RUNTIME_DIR)/lib/python2.4/site-packages/; ./configure --prefix=$(RUNTIME_DIR); make -j2; make install
	cd $(UNPACK_DIR)/openbabel-$(OPENBABEL_VERSION)/scripts/python; export CFLAGS="$$CFLAGS -m64" FC="g95 -m64" FCFLAGS="$$FCFLAGS -m64" LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:$(RUNTIME_DIR)/lib PATH=$(RUNTIME_DIR)/bin:$$PATH PYTHONPATH=$(RUNTIME_DIR)/lib/python2.4/site-packages/; $(RUNTIME_DIR)/bin/python2.4 setup.py install --prefix=$(RUNTIME_DIR)
	touch $(OPENBABEL_TARGET)
endif
